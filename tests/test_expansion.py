...
def test_horizontal_expander_subgraph_scaling_and_relational_validation():
  ...
  assert scaled["beneficiary_summary"].height == scenario_slice.bene_df.height * scale_factor
  assert scaled["carrier_claims"].height == scenario_slice.carrier_df.height * scale_factor
  assert scaled["outpatient_claims"].height == scenario_slice.outpatient_df.height * scale_factor
...
def test_cli_expand_subcommand(capsys):
  ...
  rc_vert = cli_main(["expand", "--mode", "vertical", "--scenario", "valid_baseline_cohort"])
  assert rc_vert == 0
  captured_vert = capsys.readouterr()
  assert "Vertical Expansion" in captured_vert.out

  rc_horiz = cli_main(["expand", "--mode", "horizontal", "--scenario", "valid_baseline_cohort", "--scale", "2"])
  assert rc_horiz == 0
