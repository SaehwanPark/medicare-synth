...
def test_mbsf_pde_util_manifest_and_evidence():
    ...
    pde_util_file = next(
        (f for f in manifest.files if f.file_domain == "mbsf_pde_util"), None
    )
    ...
    pde_util_constraint = next(
        (c for c in snapshot.constraints if c.constraint_id == "VAL_MBSF_PDE_UTIL_01"),
        None,
    )
...
def test_mbsf_pde_util_exporter_and_cli():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        exporter = ReleaseExporter(
            output_dir=output_dir, release_id="test_release_pde_util"
        )
