...
@patch("subprocess.run")
def test_run_autonomous_workflow_passthru_check(mock_run, tmp_path):
    """Test that passthru_check option executes Claim Pass-Through Per Diem check and records status."""
    import json
    from medicare_synth.workflow import run_autonomous_workflow
    ...
    report_file = tmp_path / "wf_report_passthru.json"
    res_code = run_autonomous_workflow(
        dry_run=True,
        json_report_path=str(report_file),
        passthru_check=True,
...
@patch("subprocess.run")
def test_run_autonomous_workflow_coinsurance_amt_check(mock_run, tmp_path):
    """Test that coinsurance_amt_check option executes Claim Coinsurance Amount check and records status."""
    import json
    from medicare_synth.workflow import run_autonomous_workflow
    ...
    report_file = tmp_path / "wf_report_coinsrnc_amt.json"
    res_code = run_autonomous_workflow(
        dry_run=True,
        json_report_path=str(report_file),
    ...
    assert data["coinsurance_amt_check"] is True


@patch("subprocess.run")
def test_run_autonomous_workflow_deductible_amt_check(mock_run, tmp_path):
    """Test that deductible_amt_check option executes Claim Beneficiary Deductible Amount check and records status."""
    import json
    from medicare_synth.workflow import run_autonomous_workflow
    ...
    report_file = tmp_path / "wf_report_deductible_amt.json"
    res_code = run_autonomous_workflow(
        dry_run=True,
        json_report_path=str(report_file),
    ...
    assert data["deductible_amt_check"] is True


@patch("subprocess.run")
def test_run_autonomous_workflow_primary_payer_paid_amt_check(mock_run, tmp_path):
    """Test that primary_payer_paid_amt_check option executes Claim Primary Payer Paid Amount check and records status."""
    import json
    from medicare_synth.workflow import run_autonomous_workflow
    ...
    report_file = tmp_path / "wf_report_primary_payer_paid_amt.json"
    res_code = run_autonomous_workflow(
        dry_run=True,
        json_report_path=str(report_file),
...
@patch("subprocess.run")
def test_run_autonomous_workflow_total_charge_amt_check(mock_run, tmp_path):
    """Test that total_charge_amt_check option executes Claim Total Charge Amount check and records status."""
    import json
    from medicare_synth.workflow import run_autonomous_workflow
    ...
    report_file = tmp_path / "wf_report_total_charge_amt.json"
    res_code = run_autonomous_workflow(
        dry_run=True,
        json_report_path=str(report_file),
    ...
    assert data["total_charge_amt_check"] is True


@patch("subprocess.run")
def test_run_autonomous_workflow_non_covered_charge_amt_check(mock_run, tmp_path):
    """Test that non_covered_charge_amt_check option executes Claim Non-Covered Charge Amount check and records status."""
    import json
    from medicare_synth.workflow import run_autonomous_workflow
    ...
    report_file = tmp_path / "wf_report_non_covered_charge_amt.json"
    res_code = run_autonomous_workflow(
        dry_run=True,
        json_report_path=str(report_file),
    ...
    assert data["non_covered_charge_amt_check"] is True


@patch("subprocess.run")
def test_run_autonomous_workflow_outlier_payment_amt_check(mock_run, tmp_path):
    """Test that outlier_payment_amt_check option executes Claim Outlier Payment Amount check and records status."""
    import json
    from medicare_synth.workflow import run_autonomous_workflow
    ...
    report_file = tmp_path / "wf_report_outlier_payment_amt.json"
    res_code = run_autonomous_workflow(
        dry_run=True,
        json_report_path=str(report_file),
    ...
    assert data["outlier_payment_amt_check"] is True


@patch("subprocess.run")
def test_run_autonomous_workflow_line_allowed_charge_amt_check(mock_run, tmp_path):
    """Test that line_allowed_charge_amt_check option executes Claim Line Allowed Charge Amount check and records status."""
    import json
    from medicare_synth.workflow import run_autonomous_workflow
    ...
    report_file = tmp_path / "wf_report_line_allowed_charge_amt.json"
    res_code = run_autonomous_workflow(
        dry_run=True,
        json_report_path=str(report_file),
    ...
    assert data["line_allowed_charge_amt_check"] is True


@patch("subprocess.run")
def test_run_autonomous_workflow_line_coinsurance_amt_check(mock_run, tmp_path):
    """Test that line_coinsurance_amt_check option executes Claim Line Coinsurance Amount check and records status."""
    import json
    from medicare_synth.workflow import run_autonomous_workflow
    ...
    report_file = tmp_path / "wf_report_line_coinsurance_amt.json"
    res_code = run_autonomous_workflow(
        dry_run=True,
        json_report_path=str(report_file),
...
def test_run_autonomous_workflow_pps_capital_check(mock_run, tmp_path):
    ...
    report_file = tmp_path / "wf_report_pps_capital.json"
    res_code = run_autonomous_workflow(
        dry_run=True,
        json_report_path=str(report_file),
    ...
    assert data["pps_capital_check"] is True


@patch("subprocess.run")
def test_run_autonomous_workflow_pps_capital_outlier_check(mock_run, tmp_path):
    """Test that pps_capital_outlier_check option executes Claim PPS Capital Outlier Amount check and records status."""
    import json
    from medicare_synth.workflow import run_autonomous_workflow
    ...
    report_file = tmp_path / "wf_report_pps_capital_outlier.json"
    res_code = run_autonomous_workflow(
        dry_run=True,
        json_report_path=str(report_file),
    ...
    assert data["pps_capital_outlier_check"] is True


@patch("subprocess.run")
def test_run_autonomous_workflow_pps_capital_ime_check(mock_run, tmp_path):
    """Test that pps_capital_ime_check option executes Claim PPS Capital IME Amount check and records status."""
    import json
    from medicare_synth.workflow import run_autonomous_workflow
    ...
    report_file = tmp_path / "wf_report_pps_capital_ime.json"
    res_code = run_autonomous_workflow(
        dry_run=True,
        json_report_path=str(report_file),
    ...
    assert data["pps_capital_ime_check"] is True


@patch("subprocess.run")
def test_run_autonomous_workflow_pps_capital_dsh_check(mock_run, tmp_path):
    """Test that pps_capital_dsh_check option executes Claim PPS Capital DSH Amount check and records status."""
    import json
    from medicare_synth.workflow import run_autonomous_workflow
    ...
    report_file = tmp_path / "wf_report_pps_capital_dsh.json"
    res_code = run_autonomous_workflow(
        dry_run=True,
        json_report_path=str(report_file),
    ...
    assert data["pps_capital_dsh_check"] is True


@patch("subprocess.run")
def test_run_autonomous_workflow_pps_capital_fsp_check(mock_run, tmp_path):
    """Test that pps_capital_fsp_check option executes Claim PPS Capital FSP Amount check and records status."""
    import json
    from medicare_synth.workflow import run_autonomous_workflow
    ...
    report_file = tmp_path / "wf_report_pps_capital_fsp.json"
    res_code = run_autonomous_workflow(
        dry_run=True,
        json_report_path=str(report_file),
    ...
    assert data["pps_capital_fsp_check"] is True


@patch("subprocess.run")
def test_run_autonomous_workflow_pps_capital_exception_check(mock_run, tmp_path):
    """Test that pps_capital_exception_check option executes Claim PPS Capital Exception Amount check and records status."""
    import json
    from medicare_synth.workflow import run_autonomous_workflow
    ...
    report_file = tmp_path / "wf_report_pps_capital_exception.json"
    res_code = run_autonomous_workflow(
        dry_run=True,
        json_report_path=str(report_file),
    ...
    assert data["pps_capital_exception_check"] is True


@patch("subprocess.run")
def test_run_autonomous_workflow_pps_operating_check(mock_run, tmp_path):
    """Test that pps_operating_check option executes Claim PPS Operating Federal Payment Amount check and records status."""
    import json
    from medicare_synth.workflow import run_autonomous_workflow
    ...
    report_file = tmp_path / "wf_report_pps_operating.json"
    res_code = run_autonomous_workflow(
        dry_run=True,
        json_report_path=str(report_file),
...
@patch("subprocess.run")
def test_run_autonomous_workflow_pps_operating_hsp_check(mock_run, tmp_path):
    """Test that pps_operating_hsp_check option executes Claim PPS Operating Hospital Specific Portion Payment Amount check and records status."""
    import json
    from medicare_synth.workflow import run_autonomous_workflow
    ...
    report_file = tmp_path / "wf_report_pps_operating_hsp.json"
    res_code = run_autonomous_workflow(
        dry_run=True,
        json_report_path=str(report_file),
    ...
    assert data["pps_operating_hsp_check"] is True


@patch("subprocess.run")
def test_run_autonomous_workflow_pps_operating_ime_check(mock_run, tmp_path):
    """Test that pps_operating_ime_check option executes Claim PPS Operating Indirect Medical Education Payment Amount check and records status."""
    import json
    from medicare_synth.workflow import run_autonomous_workflow
    ...
    report_file = tmp_path / "wf_report_pps_operating_ime.json"
    res_code = run_autonomous_workflow(
        dry_run=True,
        json_report_path=str(report_file),
    ...
    assert data["pps_operating_ime_check"] is True


@patch("subprocess.run")
def test_run_autonomous_workflow_line_payment_amt_check(mock_run, tmp_path):
    """Test that line_payment_amt_check option executes Claim Line Payment Amount check and records status."""
    import json
    from medicare_synth.workflow import run_autonomous_workflow
    ...
    report_file = tmp_path / "wf_report_line_payment_amt.json"
    res_code = run_autonomous_workflow(
        dry_run=True,
        json_report_path=str(report_file),
    ...
    assert data["line_payment_amt_check"] is True


@patch("subprocess.run")
def test_run_autonomous_workflow_line_deductible_amt_check(mock_run, tmp_path):
    """Test that line_deductible_amt_check option executes Claim Line Deductible Amount check and records status."""
    import json
    from medicare_synth.workflow import run_autonomous_workflow
    ...
    report_file = tmp_path / "wf_report_line_deductible_amt.json"
    res_code = run_autonomous_workflow(
        dry_run=True,
        json_report_path=str(report_file),
    ...
    assert data["line_deductible_amt_check"] is True


@patch("subprocess.run")
def test_run_autonomous_workflow_line_submitted_charge_amt_check(mock_run, tmp_path):
    """Test that line_submitted_charge_amt_check option executes Claim Line Submitted Charge Amount check and records status."""
    import json
    from medicare_synth.workflow import run_autonomous_workflow
    ...
    report_file = tmp_path / "wf_report_line_submitted_charge_amt.json"
    res_code = run_autonomous_workflow(
        dry_run=True,
        json_report_path=str(report_file),
    ...
    assert data["line_submitted_charge_amt_check"] is True


@patch("subprocess.run")
def test_run_autonomous_workflow_line_primary_payer_paid_amt_check(mock_run, tmp_path):
    """Test that line_primary_payer_paid_amt_check option executes Claim Line Primary Payer Paid Amount check and records status."""
    import json
    from medicare_synth.workflow import run_autonomous_workflow
    ...
    report_file = tmp_path / "wf_report_line_primary_payer_paid_amt.json"
    res_code = run_autonomous_workflow(
        dry_run=True,
        json_report_path=str(report_file),
    ...
    assert data["line_primary_payer_paid_amt_check"] is True


@patch("subprocess.run")
def test_run_autonomous_workflow_line_non_covered_charge_amt_check(mock_run, tmp_path):
    """Test that line_non_covered_charge_amt_check option executes Claim Line Non-Covered Charge Amount check and records status."""
    import json
    from medicare_synth.workflow import run_autonomous_workflow
    ...
    report_file = tmp_path / "wf_report_line_non_covered_charge_amt.json"
    res_code = run_autonomous_workflow(
        dry_run=True,
        json_report_path=str(report_file),
    ...
    assert data["line_non_covered_charge_amt_check"] is True


@patch("subprocess.run")
def test_run_autonomous_workflow_line_beneficiary_payment_amt_check(mock_run, tmp_path):
    """Test that line_beneficiary_payment_amt_check option executes Claim Line Beneficiary Payment Amount check and records status."""
    import json
    from medicare_synth.workflow import run_autonomous_workflow
    ...
    report_file = tmp_path / "wf_report_line_beneficiary_payment_amt.json"
    res_code = run_autonomous_workflow(
        dry_run=True,
        json_report_path=str(report_file),
    ...
    assert data["line_beneficiary_payment_amt_check"] is True


@patch("subprocess.run")
def test_run_autonomous_workflow_line_service_count_check(mock_run, tmp_path):
    """Test that line_service_count_check option executes Claim Line Service Count check and records status."""
    import json
    from medicare_synth.workflow import run_autonomous_workflow
    ...
    report_file = tmp_path / "wf_report_line_service_count.json"
    res_code = run_autonomous_workflow(
        dry_run=True,
        json_report_path=str(report_file),
    ...
    assert data["line_service_count_check"] is True


@patch("subprocess.run")
def test_run_autonomous_workflow_line_processing_indicator_check(mock_run, tmp_path):
    """Test that line_processing_indicator_check option executes Claim Line Processing Indicator check and records status."""
    import json
    from medicare_synth.workflow import run_autonomous_workflow
    ...
    report_file = tmp_path / "wf_report_line_processing_indicator.json"
    res_code = run_autonomous_workflow(
        dry_run=True,
        json_report_path=str(report_file),
    ...
    assert data["line_processing_indicator_check"] is True


@patch("subprocess.run")
def test_run_autonomous_workflow_line_place_of_service_check(mock_run, tmp_path):
    """Test that line_place_of_service_check option executes Claim Line Place of Service check and records status."""
    import json
    from medicare_synth.workflow import run_autonomous_workflow
    ...
    report_file = tmp_path / "wf_report_line_place_of_service.json"
    res_code = run_autonomous_workflow(
        dry_run=True,
        json_report_path=str(report_file),
...
def test_run_autonomous_workflow_line_performing_physician_npi_check(
    ...
    report_file = tmp_path / "wf_report_line_npi.json"
    res_code = run_autonomous_workflow(
        dry_run=True,
        json_report_path=str(report_file),
    ...
    assert data["line_performing_physician_npi_check"] is True


@patch("subprocess.run")
def test_run_autonomous_workflow_line_hcpcs_modifier_check(mock_run, tmp_path):
    """Test that line_hcpcs_modifier_check option executes Claim Line HCPCS Initial Modifier Code check and records status."""
    import json
    from medicare_synth.workflow import run_autonomous_workflow
    ...
    report_file = tmp_path / "wf_report_line_mdfr.json"
    res_code = run_autonomous_workflow(
        dry_run=True,
        json_report_path=str(report_file),
    ...
    assert data["line_hcpcs_modifier_check"] is True


@patch("subprocess.run")
def test_run_autonomous_workflow_line_second_hcpcs_modifier_check(mock_run, tmp_path):
    """Test that line_second_hcpcs_modifier_check option executes Claim Line HCPCS Second Modifier Code check and records status."""
    import json
    from medicare_synth.workflow import run_autonomous_workflow
    ...
    report_file = tmp_path / "wf_report_line_2nd_mdfr.json"
    res_code = run_autonomous_workflow(
        dry_run=True,
        json_report_path=str(report_file),
...
@patch("subprocess.run")
def test_run_autonomous_workflow_line_fourth_hcpcs_modifier_check(mock_run, tmp_path):
    """Test that line_fourth_hcpcs_modifier_check option executes Claim Line HCPCS Fourth Modifier Code check and records status."""
    import json
    from medicare_synth.workflow import run_autonomous_workflow
    ...
    report_file = tmp_path / "wf_report_line_4th_mdfr.json"
    res_code = run_autonomous_workflow(
        dry_run=True,
        json_report_path=str(report_file),
...
def test_run_autonomous_workflow_line_rendering_physician_npi_check(mock_run, tmp_path):
    ...
    report_file = tmp_path / "wf_report_line_rndrng_npi.json"
    res_code = run_autonomous_workflow(
        dry_run=True,
        json_report_path=str(report_file),
...
@patch("subprocess.run")
def test_run_autonomous_workflow_line_revenue_center_rate_amount_check(
    mock_run, tmp_path
):
    """Test that line_revenue_center_rate_amount_check option executes Claim Line Revenue Center Rate Amount check and records status."""
    import json
    from medicare_synth.workflow import run_autonomous_workflow
    ...
    report_file = tmp_path / "wf_report_line_rev_rate.json"
    res_code = run_autonomous_workflow(
        dry_run=True,
        json_report_path=str(report_file),
    ...
    assert data["line_revenue_center_rate_amount_check"] is True
