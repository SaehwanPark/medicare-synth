...
def _write_fixture(tmp_path: Path) -> tuple[Path, Path, Path]:
    ...
    manifest = {
        ...
        "files": [
            {
                "file_id": "beneficiary_2022",
                "filename": "beneficiary_2022.csv",
                "file_domain": "beneficiary",
                "grain": "beneficiary_year",
                "format": "csv",
                "expected_record_count": 1,
                "sha256": digest(paths["beneficiary_2022.csv"]),
            ...
            },
            {
                ...
                "sha256": digest(paths["carrier.csv"]),
                "source_url": "https://example.invalid/c",
                "primary_key": ["CLM_ID", "LINE_NUM"],
                "delimiter": "|",
            },
        ],
    }
...
def test_import_preserves_carrier_lines_and_filters_service_year(
    tmp_path: Path,
) -> None:
    ...
    assert result.carrier_df.select(["clm_id", "line_num"]).rows() == [
        ("C1", 1),
        ("C1", 2),
    ]
...
def test_import_accepts_cms_source_aliases_and_month_name_dates(tmp_path: Path) -> None:
        ...
        "files": [
            {
                ...
                "format": "csv",
                "expected_record_count": 1,
                "sha256": digest(beneficiary_path),
                "source_url": "https://example.invalid/b",
                "primary_key": ["BENE_ID"],
                "delimiter": "|",
            },
    ...
    assert result.beneficiary_df.select("bene_sex_ident_cd").to_series().to_list() == [
        "1"
    ]
    ...
    carrier = pl.DataFrame(
        {
            ...
            "line_num": [1, 1],
            "bene_id": ["0001", "9999"],
            "clm_from_dt": ["2022-01-01", "2022-01-01"],
            "clm_thru_dt": ["2022-01-01", "2022-01-01"],
        }
    )
