...
from medicare_synth.models import (
  ...
  MBSFPartCRecord,
  MBSFFFSUtilizationRecord,
  MBSFPartDPDEUtilizationRecord,
...
class BaselineNormalizer:
  ...
  @staticmethod
  def normalize_mbsf_oc(records: Sequence[dict[str, Any]]) -> pl.DataFrame:
    """Normalizes raw MBSF Other Chronic Conditions dictionary records into a canonical Polars DataFrame."""
    validated = [MBSFOtherChronicConditionsRecord(**r) for r in records]
    df = pl.DataFrame([v.model_dump() for v in validated])
    if df.is_empty():
      return pl.DataFrame(
        schema={
          "bene_id": pl.String,
          "rfrnc_yr": pl.Int64,
          "sp_arthglau": pl.String,
          "sp_asthma": pl.String,
          "sp_atrialf": pl.String,
          "sp_hyperl": pl.String,
          "sp_hypert": pl.String,
          "sp_hypot": pl.String,
          "sp_osteop": pl.String,
          "val_mbsf_oc_01": pl.Float64,
        }
      )
    return df.with_columns(
      [
        pl.col("bene_id").cast(pl.String),
        pl.col("rfrnc_yr").cast(pl.Int64),
        pl.col("sp_arthglau").cast(pl.String),
        pl.col("sp_asthma").cast(pl.String),
        pl.col("sp_atrialf").cast(pl.String),
        pl.col("sp_hyperl").cast(pl.String),
        pl.col("sp_hypert").cast(pl.String),
        pl.col("sp_hypot").cast(pl.String),
        pl.col("sp_osteop").cast(pl.String),
        pl.col("val_mbsf_oc_01").cast(pl.Float64),
      ]
    )
  ...
  @staticmethod
  def normalize_mbsf_ffs_utilization(records: Sequence[dict[str, Any]]) -> pl.DataFrame:
    """Normalizes raw MBSF FFS Utilization dictionary records into a canonical Polars DataFrame."""
    validated = [MBSFFFSUtilizationRecord(**r) for r in records]
    df = pl.DataFrame([v.model_dump() for v in validated])
    if df.is_empty():
      return pl.DataFrame(
        schema={
          ...
          "rfrnc_yr": pl.Int64,
          "ip_adm_cnt": pl.Int64,
          "op_vist_cnt": pl.Int64,
          "snf_stay_cnt": pl.Int64,
          "car_srvc_cnt": pl.Int64,
          "hha_vist_cnt": pl.Int64,
          "hosp_stay_cnt": pl.Int64,
          "dme_srvc_cnt": pl.Int64,
          "val_mbsf_ffs_01": pl.Float64,
        }
      )
    return df.with_columns(
      [
        pl.col("bene_id").cast(pl.String),
        pl.col("rfrnc_yr").cast(pl.Int64),
        pl.col("ip_adm_cnt").cast(pl.Int64),
        pl.col("op_vist_cnt").cast(pl.Int64),
        pl.col("snf_stay_cnt").cast(pl.Int64),
        pl.col("car_srvc_cnt").cast(pl.Int64),
        pl.col("hha_vist_cnt").cast(pl.Int64),
        pl.col("hosp_stay_cnt").cast(pl.Int64),
        pl.col("dme_srvc_cnt").cast(pl.Int64),
        pl.col("val_mbsf_ffs_01").cast(pl.Float64),
      ]
    )

  @staticmethod
  def normalize_mbsf_pde_utilization(records: Sequence[dict[str, Any]]) -> pl.DataFrame:
    """Normalizes raw MBSF Part D PDE Utilization dictionary records into a canonical Polars DataFrame."""
    validated = [MBSFPartDPDEUtilizationRecord(**r) for r in records]
    df = pl.DataFrame([v.model_dump() for v in validated])
    if df.is_empty():
      return pl.DataFrame(
        schema={
          "bene_id": pl.String,
          "rfrnc_yr": pl.Int64,
          "pde_tot_fill_cnt": pl.Int64,
          "pde_brand_fill_cnt": pl.Int64,
          "pde_generic_fill_cnt": pl.Int64,
          "pde_tot_cst_amt": pl.Float64,
          "pde_ptnt_pay_amt": pl.Float64,
          "pde_lis_pay_amt": pl.Float64,
          "val_mbsf_pde_util_01": pl.Float64,
        }
      )
    return df.with_columns(
      [
        pl.col("bene_id").cast(pl.String),
        pl.col("rfrnc_yr").cast(pl.Int64),
        pl.col("pde_tot_fill_cnt").cast(pl.Int64),
        pl.col("pde_brand_fill_cnt").cast(pl.Int64),
        pl.col("pde_generic_fill_cnt").cast(pl.Int64),
        pl.col("pde_tot_cst_amt").cast(pl.Float64),
        pl.col("pde_ptnt_pay_amt").cast(pl.Float64),
        pl.col("pde_lis_pay_amt").cast(pl.Float64),
        pl.col("val_mbsf_pde_util_01").cast(pl.Float64),
      ]
    )
