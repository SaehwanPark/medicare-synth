...
class KAnonymityResult(BaseModel):
    ...
    qi_columns: list[str] = Field(
        ..., description="Quasi-identifier column names evaluated."
    )
    ...
    at_risk_record_count: int = Field(
        ..., description="Count of records in classes with k < 5."
    )
...
class AuditReport(BaseModel):
    ...
    scenario_name: str = Field(
        ..., description="Name of the audited scenario or dataset."
    )
    timestamp: str = Field(..., description="ISO 8601 timestamp of audit execution.")
    join_coverage: dict[str, float] = Field(
        ..., description="Relational join coverage ratios."
    )
    k_anonymity: dict[str, KAnonymityResult] = Field(
        ..., description="K-anonymity metrics by table."
    )
    column_metrics: dict[str, list[ColumnAuditMetric]] = Field(
        ..., description="Column metrics by table."
    )
...
class AuditEngine:
    ...
    def __init__(
        self, dataset: dict[str, pl.DataFrame], scenario_name: str = "custom"
    ) -> None:
        ...
        for table_name in (
            ...
            if claims_df is not None and not claims_df.is_empty():
                ...
                if claim_bene_col:
                    ...
                    coverage[f"beneficiary_{table_name}_coverage"] = (
                        round(matched / len(claim_benes), 4) if claim_benes else 1.0
                    )

        return coverage

    def compute_k_anonymity(
        self, table_name: str, qi_columns: list[str]
    ) -> KAnonymityResult | None:
    ...
    def run_audit(self) -> AuditReport:
        ...
        if "mbsf_cu" in self.dataset:
            res = self.compute_k_anonymity(
                "mbsf_cu", ["BENE_MDCR_PAY_AMT", "BENE_TOT_PAY_AMT"]
            )
            if res is not None:
        ...
        if "mbsf_d" in self.dataset:
            res = self.compute_k_anonymity(
                "mbsf_d", ["PTD_CNTRCT_ID_01", "PTD_PBP_ID_01"]
            )
            if res is not None:
        ...
        if "mbsf_base" in self.dataset:
            res = self.compute_k_anonymity(
                "mbsf_base", ["MDCR_ENTLMT_BUYIN_IND_01", "DUAL_STUS_CD_01"]
            )
            if res is not None:
        ...
        if "mbsf_ra" in self.dataset:
            res = self.compute_k_anonymity(
                "mbsf_ra", ["CMS_HCC_RISK_SCORE", "RXHCC_RISK_SCORE"]
            )
            if res is not None:
        ...
        if "mbsf_c" in self.dataset:
            res = self.compute_k_anonymity(
                "mbsf_c", ["PTC_CNTRCT_ID_01", "PTC_PBP_ID_01"]
            )
            if res is not None:
        ...
        if "mbsf_pde_util" in self.dataset:
            res = self.compute_k_anonymity(
                "mbsf_pde_util", ["PDE_TOT_FILL_CNT", "PDE_TOT_CST_AMT"]
            )
            if res is not None:
