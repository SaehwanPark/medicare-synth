...
class ScenarioEntry(BaseModel):
    ...
    description: str = Field(
        description="Detailed explanation of the scenario purpose and invariants."
    )
    is_valid: bool = Field(
        description="True if scenario represents valid data; False if intentional anomaly."
    )
    expected_findings_count: int = Field(
        description="Number of validator findings expected when validating this scenario."
    )
    target_files: List[str] = Field(
        description="Medicare file grains represented in this scenario slice."
    )
    sample_bene_count: int = Field(
        description="Number of beneficiary records in the scenario slice."
    )
    sample_claim_count: int = Field(
        description="Total number of claim records (carrier + outpatient) in the slice."
    )
...
class ScenarioCatalog:
    ...
    _CATALOG: Dict[str, ScenarioEntry] = {
        ...
        "invalid_mbsf_base_coverage_months": ScenarioEntry(
            name="invalid_mbsf_base_coverage_months",
            description="Intentional anomaly fixture containing an MBSF Base record with invalid coverage months (>12).",
            is_valid=False,
            expected_findings_count=1,
            target_files=["beneficiary_summary", "mbsf_base_enrollment"],
            sample_bene_count=1,
            sample_claim_count=1,
        ),
    ...
    @classmethod
    def export_ci_fixtures(
        cls, output_dir: str | Path, file_format: str = "parquet"
    ) -> List[Path]:
