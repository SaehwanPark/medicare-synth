...
class FidelityProfile(BaseModel):
    ...
    carrier_claim_count: int = Field(
        ..., description="Number of carrier claim line/header records"
    )
    outpatient_claim_count: int = Field(
        ..., description="Number of outpatient claim line/header records"
    )
    inpatient_claim_count: int = Field(
        default=0, description="Number of inpatient claim line/header records"
    )
    pde_count: int = Field(
        default=0, description="Number of Part D prescription drug event records"
    )
    snf_claim_count: int = Field(
        default=0, description="Number of SNF claim line/header records"
    )
    hha_claim_count: int = Field(
        default=0, description="Number of HHA claim line/header records"
    )
    dme_claim_count: int = Field(
        default=0, description="Number of DME claim line/header records"
    )
    hospice_claim_count: int = Field(
        default=0, description="Number of Hospice claim line/header records"
    )
    mbsf_cc_count: int = Field(
        default=0, description="Number of MBSF Chronic Condition records"
    )
    mbsf_cu_count: int = Field(
        default=0, description="Number of MBSF Cost & Use records"
    )
    mbsf_d_count: int = Field(default=0, description="Number of MBSF Part D records")
    mbsf_base_count: int = Field(
        default=0, description="Number of MBSF Base Enrollment records"
    )
    mbsf_oc_count: int = Field(
        default=0, description="Number of MBSF Other Chronic Condition records"
    )
    mbsf_ndi_count: int = Field(default=0, description="Number of MBSF NDI records")
    mbsf_ra_count: int = Field(
        default=0, description="Number of MBSF Risk Adjustment records"
    )
    mbsf_c_count: int = Field(default=0, description="Number of MBSF Part C records")
    mbsf_ffs_count: int = Field(
        default=0, description="Number of MBSF FFS Utilization records"
    )
    mbsf_pde_util_count: int = Field(
        default=0, description="Number of MBSF Part D PDE Utilization records"
    )
    key_uniqueness_rate: float = Field(
        ..., description="Proportion of records satisfying primary key uniqueness"
    )

    foreign_key_validity_rate: float = Field(
        ..., description="Proportion of claims linked to valid beneficiaries"
    )
    temporal_integrity_rate: float = Field(
        ..., description="Proportion of claims with valid temporal ordering"
    )
...
class ReleaseManifest(BaseModel):
    ...
    collection_id: str = Field(
        default="CMS-2021-SYN-CLAIMS",
        description="Baseline CMS synthetic collection ID",
    )
    created_at: str = Field(..., description="ISO 8601 creation timestamp")
    validation_passed: bool = Field(
        ..., description="Whether dataset passed relational validation"
    )
...
class ReleaseExporter:
    ...
    def compute_fidelity_profile(
        ...
        mbsf_pde_util_cnt = (
            mbsf_pde_util_df.height if mbsf_pde_util_df is not None else 0
        )
        total_claims = (
            carrier_df.height
            + outpatient_df.height
            + inp_count
            + pde_cnt
            + snf_cnt
            + hha_cnt
        ...
        fk_findings = [
            ...
            if f.category == FindingCategory.RELATIONAL
        ]
        temp_findings = [
            f
            for f in validation_report.findings
            if f.category == FindingCategory.TEMPORAL
        ]

        fk_validity_rate = (
            1.0 - (len(fk_findings) / max(total_claims, 1)) if fk_findings else 1.0
        )
        temp_integrity_rate = (
            1.0 - (len(temp_findings) / max(total_claims, 1)) if temp_findings else 1.0
        )
