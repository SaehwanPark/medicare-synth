...
class ScenarioCompiler:
            ...
            [
                {
                    "bene_id": "BENE_001",
                    "rfrnc_yr": 2021,
                    "cms_hcc_risk_score": 1.250,
                    "rxhcc_risk_score": 1.100,
                    "payment_count": 12,
                    "val_mbsf_ra_01": 1.0,
                },
                {
                    "bene_id": "BENE_002",
                    "rfrnc_yr": 2021,
                    "cms_hcc_risk_score": 0.850,
                    "rxhcc_risk_score": 0.920,
                    "payment_count": 12,
                    "val_mbsf_ra_01": 1.0,
                },
                {
                    "bene_id": "BENE_003",
                    "rfrnc_yr": 2021,
                    "cms_hcc_risk_score": 0.450,
                    "rxhcc_risk_score": 0.500,
                    "payment_count": 12,
                    "val_mbsf_ra_01": 1.0,
                },
            ]
        ...
        mbsf_c_df = pl.DataFrame(
            [
                ...
                },
                {
                    "bene_id": "BENE_002",
                    "rfrnc_yr": 2021,
                    "ptc_cntrct_id_01": "H0002",
                    "ptc_pbp_id_01": "002",
                    "ptc_plan_type_cd_01": "02",
                    "bene_ma_cvrage_tot_mons": 12,
                    "val_mbsf_c_01": 1.0,
                },
                {
                    "bene_id": "BENE_003",
                    "rfrnc_yr": 2021,
                    "ptc_cntrct_id_01": "H0003",
                    "ptc_pbp_id_01": "003",
                    "ptc_plan_type_cd_01": "04",
                    "bene_ma_cvrage_tot_mons": 12,
                    "val_mbsf_c_01": 1.0,
                },
            ]
    ...
    def valid_chronic_subgroup() -> ScenarioSlice:
        """Creates a valid chronic disease subgroup slice."""
        slice_data = ScenarioCompiler.valid_baseline_cohort()
        bene_sub = slice_data.bene_df.filter(
            pl.col("bene_id").is_in(["BENE_001", "BENE_002"])
        )
        carrier_sub = slice_data.carrier_df.filter(
            pl.col("bene_id").is_in(["BENE_001", "BENE_002"])
        )
        outpatient_sub = slice_data.outpatient_df.filter(
            pl.col("bene_id").is_in(["BENE_001", "BENE_002"])
        )
        inpatient_sub = slice_data.inpatient_df.filter(
            pl.col("bene_id").is_in(["BENE_001", "BENE_002"])
        )
        pde_sub = slice_data.pde_df.filter(
            pl.col("bene_id").is_in(["BENE_001", "BENE_002"])
        )
        snf_sub = slice_data.snf_df.filter(
            pl.col("bene_id").is_in(["BENE_001", "BENE_002"])
        )
        hha_sub = slice_data.hha_df.filter(
            pl.col("bene_id").is_in(["BENE_001", "BENE_002"])
        )
        dme_sub = slice_data.dme_df.filter(
            pl.col("bene_id").is_in(["BENE_001", "BENE_002"])
        )
        hospice_sub = slice_data.hospice_df.filter(
            pl.col("bene_id").is_in(["BENE_001", "BENE_002"])
        )
        mbsf_cc_sub = slice_data.mbsf_cc_df.filter(
            pl.col("bene_id").is_in(["BENE_001", "BENE_002"])
        )
        mbsf_cu_sub = slice_data.mbsf_cu_df.filter(
            pl.col("bene_id").is_in(["BENE_001", "BENE_002"])
        )
        mbsf_d_sub = slice_data.mbsf_d_df.filter(
            pl.col("bene_id").is_in(["BENE_001", "BENE_002"])
        )
        mbsf_base_sub = slice_data.mbsf_base_df.filter(
            pl.col("bene_id").is_in(["BENE_001", "BENE_002"])
        )
        mbsf_oc_sub = slice_data.mbsf_oc_df.filter(
            pl.col("bene_id").is_in(["BENE_001", "BENE_002"])
        )
        mbsf_ndi_sub = slice_data.mbsf_ndi_df.filter(
            pl.col("bene_id").is_in(["BENE_001", "BENE_002"])
        )
        mbsf_ra_sub = slice_data.mbsf_ra_df.filter(
            pl.col("bene_id").is_in(["BENE_001", "BENE_002"])
        )
        mbsf_c_sub = slice_data.mbsf_c_df.filter(
            pl.col("bene_id").is_in(["BENE_001", "BENE_002"])
        )
        mbsf_ffs_sub = slice_data.mbsf_ffs_df.filter(
            pl.col("bene_id").is_in(["BENE_001", "BENE_002"])
        )
        mbsf_pde_util_sub = slice_data.mbsf_pde_util_df.filter(
            pl.col("bene_id").is_in(["BENE_001", "BENE_002"])
        )
        return ScenarioSlice(
            bene_df=bene_sub,
    ...
    @staticmethod
    def invalid_mbsf_risk_adjustment_score() -> ScenarioSlice:
        """Creates an anomaly scenario containing an MBSF Risk Adjustment record with negative risk score (-0.5)."""
        slice_data = ScenarioCompiler.valid_baseline_cohort()
        invalid_ra = pl.DataFrame(
            [
                {
                    "bene_id": "BENE_001",
                    "rfrnc_yr": 2021,
                    "cms_hcc_risk_score": -0.500,  # Invalid negative risk score
                    "rxhcc_risk_score": 1.100,
                    "payment_count": 12,
                    "val_mbsf_ra_01": 1.0,
                }
    ...
    @staticmethod
    def invalid_mbsf_part_c_contract() -> ScenarioSlice:
        """Creates an anomaly scenario containing an MBSF Part C record with coverage months out of bounds (15)."""
        slice_data = ScenarioCompiler.valid_baseline_cohort()
        invalid_c = pl.DataFrame(
            [
                {
                    "bene_id": "BENE_001",
                    "rfrnc_yr": 2021,
                    ...
                    "ptc_pbp_id_01": "001",
                    "ptc_plan_type_cd_01": "01",
                    "bene_ma_cvrage_tot_mons": 15,  # Invalid coverage months (> 12)
                    "val_mbsf_c_01": 1.0,
                }
            ]
        )
    ...
    @classmethod
    def invalid_mbsf_ffs_utilization_count(cls) -> ScenarioSlice:
        """Creates a scenario slice containing an invalid negative FFS utilization count."""
        slice_data = cls.valid_baseline_cohort()
        invalid_ffs = slice_data.mbsf_ffs_df.with_columns(
            pl.when(pl.col("bene_id") == "BENE_001")
            .then(pl.lit(-1))
            .otherwise(pl.col("ip_adm_cnt"))
            .alias("ip_adm_cnt")
        )
        return ScenarioSlice(
            bene_df=slice_data.bene_df,
    ...
    def get_scenario(cls, name: str) -> ScenarioSlice:
        ...
        if name not in scenarios:
            raise ValueError(
                f"Unknown scenario name: '{name}'. Available: {list(scenarios.keys())}"
            )
