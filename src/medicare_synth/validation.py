...
class RelationalValidator:
    ...
    @staticmethod
    def check_mbsf_hmo_indicator_constraints(
        mbsf_base_df: pl.DataFrame,
    ) -> list[Finding]:
        """Identifies MBSF Base records with invalid monthly HMO Indicator Codes outside CCW valid set {'0','1','2','A','B','C','4','N'}.

        Per CMS CCW MBSF Base codebook, each monthly code (hmo_ind_01–12) when non-null
        must belong to the valid CCW HMO indicator set.
        """
        if mbsf_base_df.is_empty():
            return []

        hmo_cols = [f"hmo_ind_{i:02d}" for i in range(1, 13)]
        upper_hmo_cols = [f"HMO_IND_{i:02d}" for i in range(1, 13)]
        present_cols = [
            c for c in hmo_cols + upper_hmo_cols if c in mbsf_base_df.columns
        ]
        if not present_cols:
            return []

        valid_codes = {"0", "1", "2", "A", "B", "C", "4", "N"}
        invalid_conditions = []
        for col in present_cols:
            if mbsf_base_df.schema[col] == pl.Null:
                continue
        ...
        if invalid_count > 0:
            ...
            return [
                Finding(
                    ...
                    severity=Severity.HIGH,
                    message=(
                        f"Found {invalid_count} MBSF Base records with invalid monthly HMO "
                        f"Indicator Code outside CCW valid set {{'0','1','2','A','B','C','4','N'}}."
                    ),
                    count=invalid_count,
                    details={
                        "table_name": "MBSF Base Enrollment",
    ...
    @staticmethod
    def check_state_code_constraints(
        df: pl.DataFrame, table_type: str = "Beneficiary Summary"
    ) -> list[Finding]:
        """Identifies records with invalid State Code format (must be 2 numeric digits when present)."""
        if df.is_empty():
            return []
        ...
        non_null_state = df.filter(pl.col(state_col).is_not_null())
        if non_null_state.is_empty():
            return []

        invalid = non_null_state.filter(
            ~pl.col(state_col).cast(pl.Utf8).str.contains(r"^[0-9]{2}$")
        )
        invalid_count = len(invalid)
        ...
        if invalid_count > 0:
            ...
            return [
                Finding(
                    ...
                    category=FindingCategory.ADMINISTRATIVE,
                    severity=Severity.HIGH,
                    message=f"Found {invalid_count} records in {table_type} with invalid State Code format (must be 2 numeric digits).",
                    count=invalid_count,
                    details={"table_type": table_type, "sample_ids": sample_ids},
                )
            ]
    ...
    @staticmethod
    def check_claim_discharge_status_constraints(
        claim_df: pl.DataFrame, claim_type: str = "Claim Records"
    ) -> list[Finding]:
        """Identifies claims with invalid Patient Discharge Status Code format (must be 2 numeric digits when present)."""
        if claim_df.is_empty():
            return []
        ...
        non_null_status = claim_df.filter(pl.col(status_col).is_not_null())
        if non_null_status.is_empty():
            return []

        invalid = non_null_status.filter(
            ~pl.col(status_col).cast(pl.Utf8).str.contains(r"^[0-9]{2}$")
        )
        invalid_count = len(invalid)
        ...
        if invalid_count > 0:
            ...
            return [
                Finding(
                    rule_id="DISCHARGE-001",
                    category=FindingCategory.ADMINISTRATIVE,
                    severity=Severity.HIGH,
                    message=f"Found {invalid_count} claims in {claim_type} with invalid Patient Discharge Status Code format (must be 2 numeric digits).",
                    count=invalid_count,
                    details={"claim_type": claim_type, "sample_clm_ids": sample_ids},
                )
            ]
    ...
    @staticmethod
    def check_claim_admission_type_constraints(
        claim_df: pl.DataFrame, claim_type: str = "Claim Records"
    ) -> list[Finding]:
        """Identifies claims with invalid Claim Admission Type Code format (must be 1 alphanumeric character when present)."""
        if claim_df.is_empty():
            return []
        ...
        invalid = non_null_type.filter(
            ~pl.col(type_col).cast(pl.Utf8).str.contains(r"^[0-9A-Za-z]{1}$")
        )
        invalid_count = len(invalid)
        ...
        if invalid_count > 0:
            ...
            return [
                Finding(
                    rule_id="ADMSN-TYPE-001",
                    category=FindingCategory.ADMINISTRATIVE,
                    severity=Severity.HIGH,
                    message=f"Found {invalid_count} claims in {claim_type} with invalid Claim Admission Type Code format (must be 1 alphanumeric character).",
                    count=invalid_count,
                    details={"claim_type": claim_type, "sample_clm_ids": sample_ids},
                )
            ]
    ...
    @staticmethod
    def check_claim_frequency_code_constraints(
        claim_df: pl.DataFrame, claim_type: str = "Claim Records"
    ) -> list[Finding]:
        """Identifies claims with invalid Claim Frequency Code format (must be 1 alphanumeric character when present)."""
        if claim_df.is_empty():
            return []
        ...
        if invalid_count > 0:
            ...
            return [
                Finding(
                    ...
                    category=FindingCategory.ADMINISTRATIVE,
                    severity=Severity.HIGH,
                    message=f"Found {invalid_count} claims in {claim_type} with invalid Claim Frequency Code format (must be 1 alphanumeric character).",
                    count=invalid_count,
                    details={"claim_type": claim_type, "sample_clm_ids": sample_ids},
                )
            ]
    ...
    @staticmethod
    def check_claim_query_code_constraints(
        claim_df: pl.DataFrame, claim_type: str = "Claim Records"
    ) -> list[Finding]:
        """Identifies claims with invalid Claim Query Code format (must be 1 alphanumeric character when present)."""
        if claim_df.is_empty():
            return []
        ...
        if invalid_count > 0:
            ...
            return [
                Finding(
                    ...
                    category=FindingCategory.ADMINISTRATIVE,
                    severity=Severity.HIGH,
                    message=f"Found {invalid_count} claims in {claim_type} with invalid Claim Query Code format (must be 1 alphanumeric character).",
                    count=invalid_count,
                    details={"claim_type": claim_type, "sample_clm_ids": sample_ids},
                )
            ]
    ...
    @staticmethod
    def check_claim_pps_capital_constraints(
        claim_df: pl.DataFrame, claim_type: str = "Claim Records"
    ) -> list[Finding]:
        """Identifies claims with negative Claim PPS Capital Federal Payment Amount (< 0 when present)."""
        if claim_df.is_empty():
            return []

        amt_col = None
        for col in [
            "clm_pps_cptl_fdrle_pmt_amt",
            "CLM_PPS_CPTL_FDRLE_PMT_AMT",
            "pps_cptl_fdrle_pmt_amt",
            "PPS_CPTL_FDRLE_PMT_AMT",
            "pps_capital_fdrle_amt",
            "PPS_CAPITAL_FDRLE_AMT",
        ]:
            if col in claim_df.columns:
                amt_col = col
        ...
        if invalid_count > 0:
            ...
            return [
                Finding(
                    rule_id="PPS-CAPITAL-001",
                    category=FindingCategory.ADMINISTRATIVE,
                    severity=Severity.HIGH,
                    message=f"Found {invalid_count} claims in {claim_type} with negative PPS Capital Federal Payment Amount (< 0).",
                    count=invalid_count,
                    details={"claim_type": claim_type, "sample_clm_ids": sample_ids},
                )
            ]
    ...
    @staticmethod
    def check_claim_pps_capital_outlier_constraints(
        claim_df: pl.DataFrame, claim_type: str = "Claim Records"
    ) -> list[Finding]:
        """Identifies claims with negative Claim PPS Capital Outlier Amount (< 0 when present)."""
        if claim_df.is_empty():
            return []
        ...
        if invalid_count > 0:
            ...
            return [
                Finding(
                    rule_id="PPS-OUTLIER-001",
                    category=FindingCategory.ADMINISTRATIVE,
                    severity=Severity.HIGH,
                    message=f"Found {invalid_count} claims in {claim_type} with negative PPS Capital Outlier Amount (< 0).",
                    count=invalid_count,
                    details={"claim_type": claim_type, "sample_clm_ids": sample_ids},
                )
            ]
    ...
    @staticmethod
    def check_claim_pps_capital_ime_amount_constraints(
        claim_df: pl.DataFrame, claim_type: str = "Claim Records"
    ) -> list[Finding]:
        """Identifies claims with negative Claim PPS Capital IME Amount (< 0 when present)."""
        if claim_df.is_empty():
            return []
        ...
        if invalid_count > 0:
            ...
            return [
                Finding(
                    ...
                    category=FindingCategory.ADMINISTRATIVE,
                    severity=Severity.HIGH,
                    message=f"Found {invalid_count} claims in {claim_type} with negative PPS Capital IME Amount (< 0).",
                    count=invalid_count,
                    details={"claim_type": claim_type, "sample_clm_ids": sample_ids},
                )
            ]
    ...
    @staticmethod
    def check_claim_pps_capital_dsh_amount_constraints(
        claim_df: pl.DataFrame, claim_type: str = "Claim Records"
    ) -> list[Finding]:
        """Identifies claims with negative Claim PPS Capital DSH Adjustment Amount (< 0 when present)."""
        if claim_df.is_empty():
            return []
        ...
        if invalid_count > 0:
            ...
            return [
                Finding(
                    ...
                    category=FindingCategory.ADMINISTRATIVE,
                    severity=Severity.HIGH,
                    message=f"Found {invalid_count} claims in {claim_type} with negative PPS Capital DSH Amount (< 0).",
                    count=invalid_count,
                    details={"claim_type": claim_type, "sample_clm_ids": sample_ids},
                )
            ]
    ...
    @staticmethod
    def check_claim_pps_capital_fsp_amount_constraints(
        claim_df: pl.DataFrame, claim_type: str = "Claim Records"
    ) -> list[Finding]:
        """Identifies claims with negative Claim PPS Capital Hospital Specific Portion Payment Amount (< 0 when present)."""
        if claim_df.is_empty():
            return []
        ...
        if invalid_count > 0:
            ...
            return [
                Finding(
                    ...
                    category=FindingCategory.ADMINISTRATIVE,
                    severity=Severity.HIGH,
                    message=f"Found {invalid_count} claims in {claim_type} with negative PPS Capital FSP Amount (< 0).",
                    count=invalid_count,
                    details={"claim_type": claim_type, "sample_clm_ids": sample_ids},
                )
            ]
    ...
    @staticmethod
    def check_claim_pps_capital_exception_amount_constraints(
        claim_df: pl.DataFrame, claim_type: str = "Claim Records"
    ) -> list[Finding]:
        """Identifies claims with negative Claim PPS Capital Exception Amount (< 0 when present)."""
        if claim_df.is_empty():
            return []
        ...
        if invalid_count > 0:
            ...
            return [
                Finding(
                    rule_id="PPS-EXCPTN-001",
                    category=FindingCategory.ADMINISTRATIVE,
                    severity=Severity.HIGH,
                    message=f"Found {invalid_count} claims in {claim_type} with negative PPS Capital Exception Amount (< 0).",
                    count=invalid_count,
                    details={"claim_type": claim_type, "sample_clm_ids": sample_ids},
                )
            ]
    ...
    @staticmethod
    def check_claim_pps_operating_federal_payment_amount_constraints(
        claim_df: pl.DataFrame, claim_type: str = "Claim Records"
    ) -> list[Finding]:
        """Identifies claims with negative Claim PPS Operating Federal Payment Amount (< 0 when present)."""
        if claim_df.is_empty():
            return []

        amt_col = None
        for col in [
            "clm_pps_oprtg_fdrle_pmt_amt",
            "CLM_PPS_OPRTG_FDRLE_PMT_AMT",
            "pps_oprtg_fdrle_pmt_amt",
            "PPS_OPRTG_FDRLE_PMT_AMT",
            "pps_operating_federal_pmt_amt",
            "PPS_OPERATING_FEDERAL_PMT_AMT",
        ]:
            if col in claim_df.columns:
                amt_col = col
        ...
        if invalid_count > 0:
            ...
            return [
                Finding(
                    rule_id="PPS-OPRTG-001",
                    category=FindingCategory.ADMINISTRATIVE,
                    severity=Severity.HIGH,
                    message=f"Found {invalid_count} claims in {claim_type} with negative PPS Operating Federal Payment Amount (< 0).",
                    count=invalid_count,
                    details={"claim_type": claim_type, "sample_clm_ids": sample_ids},
                )
            ]
    ...
    @staticmethod
    def check_claim_pps_operating_outlier_amount_constraints(
        claim_df: pl.DataFrame, claim_type: str = "Claim Records"
    ) -> list[Finding]:
        """Identifies claims with negative Claim PPS Operating Outlier Amount (< 0 when present)."""
        if claim_df.is_empty():
            return []

        amt_col = None
        for col in [
            "clm_pps_oprtg_outlier_amt",
            "CLM_PPS_OPRTG_OUTLIER_AMT",
            "pps_oprtg_outlier_amt",
            "PPS_OPRTG_OUTLIER_AMT",
            "pps_operating_outlier_amt",
            "PPS_OPERATING_OUTLIER_AMT",
        ]:
            if col in claim_df.columns:
                amt_col = col
        ...
        if invalid_count > 0:
            ...
            return [
                Finding(
                    rule_id="PPS-OPRTG-002",
                    category=FindingCategory.ADMINISTRATIVE,
                    severity=Severity.HIGH,
                    message=f"Found {invalid_count} claims in {claim_type} with negative PPS Operating Outlier Amount (< 0).",
                    count=invalid_count,
                    details={"claim_type": claim_type, "sample_clm_ids": sample_ids},
                )
            ]
    ...
    @staticmethod
    def check_claim_pps_operating_hsp_payment_amount_constraints(
        claim_df: pl.DataFrame, claim_type: str = "Claim Records"
    ) -> list[Finding]:
        """Identifies claims with negative Claim PPS Operating Hospital Specific Portion Payment Amount (< 0 when present)."""
        if claim_df.is_empty():
            return []

        amt_col = None
        for col in [
            "clm_pps_oprtg_hsp_pmt_amt",
            "CLM_PPS_OPRTG_HSP_PMT_AMT",
            "pps_oprtg_hsp_pmt_amt",
            "PPS_OPRTG_HSP_PMT_AMT",
            "pps_operating_hsp_pmt_amt",
            "PPS_OPERATING_HSP_PMT_AMT",
        ]:
            if col in claim_df.columns:
                amt_col = col
        ...
        if invalid_count > 0:
            ...
            return [
                Finding(
                    rule_id="PPS-OPRTG-003",
                    category=FindingCategory.ADMINISTRATIVE,
                    severity=Severity.HIGH,
                    message=f"Found {invalid_count} claims in {claim_type} with negative PPS Operating Hospital Specific Portion Payment Amount (< 0).",
                    count=invalid_count,
                    details={"claim_type": claim_type, "sample_clm_ids": sample_ids},
                )
            ]
    ...
    @staticmethod
    def check_claim_pps_operating_dsh_amount_constraints(
        claim_df: pl.DataFrame, claim_type: str = "Claim Records"
    ) -> list[Finding]:
        """Identifies claims with negative Claim PPS Operating Disproportionate Share Hospital Adjustment Amount (< 0 when present)."""
        if claim_df.is_empty():
            return []

        amt_col = None
        for col in [
            "clm_pps_oprtg_dsh_amt",
            "CLM_PPS_OPRTG_DSH_AMT",
            "pps_oprtg_dsh_amt",
            "PPS_OPRTG_DSH_AMT",
            "pps_operating_dsh_amt",
            "PPS_OPERATING_DSH_AMT",
        ]:
            if col in claim_df.columns:
                amt_col = col
        ...
        if invalid_count > 0:
            ...
            return [
                Finding(
                    rule_id="PPS-OPRTG-005",
                    category=FindingCategory.ADMINISTRATIVE,
                    severity=Severity.HIGH,
                    message=f"Found {invalid_count} claims in {claim_type} with negative PPS Operating Disproportionate Share Hospital Adjustment Amount (< 0).",
                    count=invalid_count,
                    details={"claim_type": claim_type, "sample_clm_ids": sample_ids},
                )
            ]
    ...
    @staticmethod
    def check_claim_coinsurance_amount_constraints(
        claim_df: pl.DataFrame, claim_type: str = "Claim Records"
    ) -> list[Finding]:
        """Identifies claims with negative Claim Coinsurance Amount (< 0)."""
        if claim_df.is_empty():
            return []

        col_name = None
        for col in [
            "nch_clm_coinsrnc_amt",
            "NCH_CLM_COINSRNC_AMT",
            "clm_coinsrnc_amt",
            "CLM_COINSRNC_AMT",
            "coinsrnc_amt",
        ...
        if invalid_count > 0:
            ...
            return [
                Finding(
                    rule_id="COINSRNC-AMT-001",
                    category=FindingCategory.ADMINISTRATIVE,
                    severity=Severity.HIGH,
                    message=f"Found {invalid_count} claims in {claim_type} with negative Coinsurance Amount (< 0).",
                    count=invalid_count,
                    details={"claim_type": claim_type, "sample_clm_ids": sample_ids},
                )
            ]
    ...
    @staticmethod
    def check_claim_deductible_amount_constraints(
        claim_df: pl.DataFrame, claim_type: str = "Claim Records"
    ) -> list[Finding]:
        """Identifies claims with negative Claim Beneficiary Deductible Amount (< 0)."""
        if claim_df.is_empty():
            return []

        col_name = None
        for col in [
            "nch_clm_bene_ddctbl_amt",
            "NCH_CLM_BENE_DDCTBL_AMT",
            "clm_bene_ddctbl_amt",
            "CLM_BENE_DDCTBL_AMT",
            "bene_ddctbl_amt",
        ...
        if invalid_count > 0:
            ...
            return [
                Finding(
                    ...
                    category=FindingCategory.ADMINISTRATIVE,
                    severity=Severity.HIGH,
                    message=f"Found {invalid_count} claims in {claim_type} with negative Claim Beneficiary Deductible Amount (< 0).",
                    count=invalid_count,
                    details={"claim_type": claim_type, "sample_clm_ids": sample_ids},
                )
            ]
    ...
    @staticmethod
    def check_claim_primary_payer_paid_amount_constraints(
        claim_df: pl.DataFrame, claim_type: str = "Claim Records"
    ) -> list[Finding]:
        """Identifies claims with negative Claim Primary Payer Paid Amount (< 0)."""
        if claim_df.is_empty():
            return []
        ...
        if invalid_count > 0:
            ...
            return [
                Finding(
                    rule_id="PRPAY-AMT-001",
                    category=FindingCategory.ADMINISTRATIVE,
                    severity=Severity.HIGH,
                    message=f"Found {invalid_count} claims in {claim_type} with negative Primary Payer Paid Amount (< 0).",
                    count=invalid_count,
                    details={"claim_type": claim_type, "sample_clm_ids": sample_ids},
                )
            ]
    ...
    @staticmethod
    def check_claim_payment_amount_constraints(
        claim_df: pl.DataFrame, claim_type: str = "Claim Records"
    ) -> list[Finding]:
        """Identifies claims with negative Claim Medicare Payment Amount (< 0)."""
        if claim_df.is_empty():
            return []
        ...
        if invalid_count > 0:
            ...
            return [
                Finding(
                    ...
                    category=FindingCategory.ADMINISTRATIVE,
                    severity=Severity.HIGH,
                    message=f"Found {invalid_count} claims in {claim_type} with negative Claim Medicare Payment Amount (< 0).",
                    count=invalid_count,
                    details={"claim_type": claim_type, "sample_clm_ids": sample_ids},
                )
            ]
    ...
    @staticmethod
    def check_claim_outlier_payment_amount_constraints(
        claim_df: pl.DataFrame, claim_type: str = "Claim Records"
    ) -> list[Finding]:
        """Identifies claims with negative Claim Outlier Payment Amount (< 0)."""
        if claim_df.is_empty():
            return []
        ...
        if invalid_count > 0:
            ...
            return [
                Finding(
                    rule_id="OUTLIER-PMT-001",
                    category=FindingCategory.ADMINISTRATIVE,
                    severity=Severity.HIGH,
                    message=f"Found {invalid_count} claims in {claim_type} with negative Claim Outlier Payment Amount (< 0).",
                    count=invalid_count,
                    details={"claim_type": claim_type, "sample_clm_ids": sample_ids},
                )
            ]
    ...
    @staticmethod
    def check_claim_total_charge_amount_constraints(
        claim_df: pl.DataFrame, claim_type: str = "Claim Records"
    ) -> list[Finding]:
        """Identifies claims with negative Claim Total Charge Amount (< 0)."""
        if claim_df.is_empty():
            return []
        ...
        if invalid_count > 0:
            ...
            return [
                Finding(
                    rule_id="TOTCHRG-AMT-001",
                    category=FindingCategory.ADMINISTRATIVE,
                    severity=Severity.HIGH,
                    message=f"Found {invalid_count} claims in {claim_type} with negative Claim Total Charge Amount (< 0).",
                    count=invalid_count,
                    details={"claim_type": claim_type, "sample_clm_ids": sample_ids},
                )
            ]
    ...
    def check_claim_line_allowed_charge_amount_constraints(
        ...
        """Identifies claim line items with negative Claim Line Allowed Charge Amount (< 0)."""
        if claim_df.is_empty():
            return []

        col_name = None
        for col in [
            "line_allowd_chrg_amt",
            "LINE_ALLOWD_CHRG_AMT",
            "line_aloowd_chrg_amt",
            "LINE_ALOOWD_CHRG_AMT",
            "line_allowed_amt",
            "LINE_ALLOWED_AMT",
            "allowd_chrg_amt",
        ...
        if invalid_count > 0:
            ...
            return [
                Finding(
                    rule_id="LINE-ALLOWD-AMT-001",
                    category=FindingCategory.ADMINISTRATIVE,
                    severity=Severity.HIGH,
                    message=f"Found {invalid_count} records in {claim_type} with negative Line Allowed Charge Amount (< 0).",
                    count=invalid_count,
                    details={"claim_type": claim_type, "sample_clm_ids": sample_ids},
                )
            ]
    ...
    @staticmethod
    def check_claim_line_coinsurance_amount_constraints(
        claim_df: pl.DataFrame, claim_type: str = "Claim Line Items"
    ) -> list[Finding]:
        """Identifies claim line items with negative Claim Line Coinsurance Amount (< 0)."""
        if claim_df.is_empty():
            return []
        ...
        for col in [
            "line_coinsrnc_amt",
            "LINE_COINSRNC_AMT",
            "line_nch_bene_ptb_coinsrnc_amt",
            "LINE_NCH_BENE_PTB_COINSRNC_AMT",
            "line_coinsrnc",
            "LINE_COINSRNC",
        ]:
        ...
        if invalid_count > 0:
            ...
            return [
                Finding(
                    rule_id="LINE-COINSRNC-AMT-001",
                    category=FindingCategory.ADMINISTRATIVE,
                    severity=Severity.HIGH,
                    message=f"Found {invalid_count} records in {claim_type} with negative Line Coinsurance Amount (< 0).",
                    count=invalid_count,
                    details={"claim_type": claim_type, "sample_clm_ids": sample_ids},
                )
            ]
    ...
    @staticmethod
    def check_claim_line_payment_amount_constraints(
        claim_df: pl.DataFrame, claim_type: str = "Claim Line Items"
    ) -> list[Finding]:
        """Identifies claim line items with negative Claim Line NCH Payment Amount (< 0)."""
        if claim_df.is_empty():
            return []
        ...
        if invalid_count > 0:
            ...
            return [
                Finding(
                    rule_id="LINE-PMT-AMT-001",
                    category=FindingCategory.ADMINISTRATIVE,
                    severity=Severity.HIGH,
                    message=f"Found {invalid_count} records in {claim_type} with negative Line NCH Payment Amount (< 0).",
                    count=invalid_count,
                    details={"claim_type": claim_type, "sample_clm_ids": sample_ids},
                )
            ]
    ...
    @staticmethod
    def check_claim_line_deductible_amount_constraints(
        claim_df: pl.DataFrame, claim_type: str = "Claim Line Items"
    ) -> list[Finding]:
        """Identifies claim line items with negative Claim Line Beneficiary Deductible Amount (< 0)."""
        if claim_df.is_empty():
            return []
        ...
        if invalid_count > 0:
            ...
            return [
                Finding(
                    rule_id="LINE-DDCTBL-AMT-001",
                    category=FindingCategory.ADMINISTRATIVE,
                    severity=Severity.HIGH,
                    message=f"Found {invalid_count} records in {claim_type} with negative Line Deductible Amount (< 0).",
                    count=invalid_count,
                    details={"claim_type": claim_type, "sample_clm_ids": sample_ids},
                )
            ]
    ...
    @staticmethod
    def check_claim_line_primary_payer_paid_amount_constraints(
        claim_df: pl.DataFrame, claim_type: str = "Claim Line Items"
    ) -> list[Finding]:
        """Identifies claim line items with negative Claim Line Primary Payer Paid Amount (< 0)."""
        if claim_df.is_empty():
            return []
        ...
        if invalid_count > 0:
            ...
            return [
                Finding(
                    rule_id="LINE-PRPAY-AMT-001",
                    category=FindingCategory.ADMINISTRATIVE,
                    severity=Severity.HIGH,
                    message=f"Found {invalid_count} records in {claim_type} with negative Line Primary Payer Paid Amount (< 0).",
                    count=invalid_count,
                    details={"claim_type": claim_type, "sample_clm_ids": sample_ids},
                )
            ]
    ...
    @staticmethod
    def check_claim_line_non_covered_charge_amount_constraints(
        claim_df: pl.DataFrame, claim_type: str = "Claim Line Items"
    ) -> list[Finding]:
        """Identifies claim line items with negative Claim Line Non-Covered Charge Amount (< 0)."""
        if claim_df.is_empty():
            return []
        ...
        for col in [
            "line_ncvd_chrg_amt",
            "LINE_NCVD_CHRG_AMT",
            "line_non_covered_charge_amt",
            "LINE_NON_COVERED_CHARGE_AMT",
            "line_nch_non_cvrd_chrg_amt",
            "LINE_NCH_NON_CVRD_CHRG_AMT",
        ]:
            if col in claim_df.columns:
                col_name = col
        ...
        if invalid_count > 0:
            ...
            return [
                Finding(
                    rule_id="LINE-NCVD-CHRG-001",
                    category=FindingCategory.ADMINISTRATIVE,
                    severity=Severity.HIGH,
                    message=f"Found {invalid_count} records in {claim_type} with negative Line Non-Covered Charge Amount (< 0).",
                    count=invalid_count,
                    details={"claim_type": claim_type, "sample_clm_ids": sample_ids},
                )
            ]
    ...
    @staticmethod
    def check_claim_line_beneficiary_payment_amount_constraints(
        claim_df: pl.DataFrame, claim_type: str = "Claim Line Items"
    ) -> list[Finding]:
        """Identifies claim line items with negative Claim Line Beneficiary Payment Amount (< 0)."""
        if claim_df.is_empty():
            return []
        ...
        for col in [
            "line_bene_pmt_amt",
            "LINE_BENE_PMT_AMT",
            "line_bene_payment_amt",
            "LINE_BENE_PAYMENT_AMT",
            "line_bene_pd_amt",
            "LINE_BENE_PD_AMT",
            "line_beneficiary_pmt_amt",
            "LINE_BENEFICIARY_PMT_AMT",
        ]:
            if col in claim_df.columns:
                col_name = col
        ...
        if invalid_count > 0:
            ...
            return [
                Finding(
                    rule_id="LINE-BENE-PMT-001",
                    category=FindingCategory.ADMINISTRATIVE,
                    severity=Severity.HIGH,
                    message=f"Found {invalid_count} records in {claim_type} with negative Line Beneficiary Payment Amount (< 0).",
                    count=invalid_count,
                    details={"claim_type": claim_type, "sample_clm_ids": sample_ids},
                )
            ]
    ...
    @staticmethod
    def check_claim_line_place_of_service_constraints(
        claim_df: pl.DataFrame, claim_type: str = "Claim Line Items"
    ) -> list[Finding]:
        """Identifies claim line items with invalid Claim Line Place of Service Code format (must be 2 numeric digits when present)."""
        if claim_df.is_empty():
            return []
        ...
        if invalid_count > 0:
            ...
            return [
                Finding(
                    ...
                    category=FindingCategory.ADMINISTRATIVE,
                    severity=Severity.HIGH,
                    message=f"Found {invalid_count} records in {claim_type} with invalid Claim Line Place of Service Code format (must be 2 numeric digits).",
                    count=invalid_count,
                    details={"claim_type": claim_type, "sample_clm_ids": sample_ids},
                )
            ]
    ...
    @staticmethod
    def check_claim_line_type_of_service_constraints(
        claim_df: pl.DataFrame, claim_type: str = "Claim Line Items"
    ) -> list[Finding]:
        """Identifies claim line items with invalid Claim Line Type of Service Code format (must be 1 to 2 alphanumeric characters when present)."""
        if claim_df.is_empty():
            return []
        ...
        non_null_tos = claim_df.filter(pl.col(tos_col).is_not_null())
        if non_null_tos.is_empty():
            return []

        invalid = non_null_tos.filter(
            ~pl.col(tos_col).cast(pl.Utf8).str.contains(r"^[0-9A-Za-z]{1,2}$")
        )
        invalid_count = len(invalid)
        ...
        if invalid_count > 0:
            ...
            return [
                Finding(
                    ...
                    category=FindingCategory.ADMINISTRATIVE,
                    severity=Severity.HIGH,
                    message=f"Found {invalid_count} records in {claim_type} with invalid Claim Line Type of Service Code format (must be 1 to 2 alphanumeric characters).",
                    count=invalid_count,
                    details={"claim_type": claim_type, "sample_clm_ids": sample_ids},
                )
            ]
    ...
    @staticmethod
    def check_claim_line_hcpcs_modifier_constraints(
        claim_df: pl.DataFrame, claim_type: str = "Claim Line Items"
    ) -> list[Finding]:
        """Identifies claim line items with invalid HCPCS Initial Modifier Code format (must be 2 alphanumeric characters when present)."""
        if claim_df.is_empty():
            return []
        ...
        if invalid_count > 0:
            ...
            return [
                Finding(
                    ...
                    category=FindingCategory.ADMINISTRATIVE,
                    severity=Severity.HIGH,
                    message=f"Found {invalid_count} records in {claim_type} with invalid Claim Line HCPCS Initial Modifier Code format (must be 2 alphanumeric characters).",
                    count=invalid_count,
                    details={"claim_type": claim_type, "sample_clm_ids": sample_ids},
                )
            ]
    ...
    @staticmethod
    def check_claim_line_second_hcpcs_modifier_constraints(
        claim_df: pl.DataFrame, claim_type: str = "Claim Line Items"
    ) -> list[Finding]:
        """Identifies claim line items with invalid HCPCS Second Modifier Code format (must be 2 alphanumeric characters when present)."""
        if claim_df.is_empty():
            return []
        ...
        if invalid_count > 0:
            ...
            return [
                Finding(
                    ...
                    category=FindingCategory.ADMINISTRATIVE,
                    severity=Severity.HIGH,
                    message=f"Found {invalid_count} records in {claim_type} with invalid Claim Line HCPCS Second Modifier Code format (must be 2 alphanumeric characters).",
                    count=invalid_count,
                    details={"claim_type": claim_type, "sample_clm_ids": sample_ids},
                )
            ]
    ...
    @staticmethod
    def check_claim_line_fourth_hcpcs_modifier_constraints(
        claim_df: pl.DataFrame, claim_type: str = "Claim Line Items"
    ) -> list[Finding]:
        """Identifies claim line items with invalid HCPCS Fourth Modifier Code format (must be 2 alphanumeric characters when present)."""
        if claim_df.is_empty():
            return []
        ...
        if invalid_count > 0:
            ...
            return [
                Finding(
                    ...
                    category=FindingCategory.ADMINISTRATIVE,
                    severity=Severity.HIGH,
                    message=f"Found {invalid_count} records in {claim_type} with invalid Claim Line HCPCS Fourth Modifier Code format (must be 2 alphanumeric characters).",
                    count=invalid_count,
                    details={"claim_type": claim_type, "sample_clm_ids": sample_ids},
                )
            ]
    ...
    def check_claim_line_rendering_physician_npi_constraints(
        ...
        """Identifies claim line items with invalid Rendering/Ordering Physician NPI format (must be 10 numeric digits when present).

        Covers rndrng_physn_npi (rendering), op_physn_npi (ordering), and othr_physn_npi (other)
        aliases used across carrier, outpatient, and DME claim lines.
        """
        if claim_df.is_empty():
            return []
        ...
        if invalid_count > 0:
            ...
            return [
                Finding(
                    rule_id="LINE-NPI-002",
                    category=FindingCategory.ADMINISTRATIVE,
                    severity=Severity.HIGH,
                    message=f"Found {invalid_count} records in {claim_type} with invalid Claim Line Rendering/Ordering Physician NPI format (must be 10 numeric digits).",
                    count=invalid_count,
                    details={"claim_type": claim_type, "sample_clm_ids": sample_ids},
                )
            ]
    ...
    @staticmethod
    def check_claim_line_ordering_physician_npi_constraints(
        claim_df: pl.DataFrame, claim_type: str = "Claim Line Items"
    ) -> list[Finding]:
        """Identifies claim line items with invalid Ordering Physician NPI format (must be 10 numeric digits when present)."""
        if claim_df.is_empty():
            return []
    ...
    @staticmethod
    def check_claim_line_revenue_center_rate_amount_constraints(
        claim_df: pl.DataFrame, claim_type: str = "Claim Line Records"
    ) -> list[Finding]:
        """Identifies claim line items with negative Revenue Center Rate Amount (< 0 when present)."""
        if claim_df.is_empty():
            return []
        ...
        if invalid_count > 0:
            ...
            return [
                Finding(
                    rule_id="REV-RATE-001",
                    category=FindingCategory.ADMINISTRATIVE,
                    severity=Severity.HIGH,
                    message=f"Found {invalid_count} records in {claim_type} with negative Revenue Center Rate Amount (< 0).",
                    count=invalid_count,
                    details={"claim_type": claim_type, "sample_clm_ids": sample_ids},
                )
            ]
    ...
    def validate_slice(
        ...
        if carrier_df is not None and not carrier_df.is_empty():
            ...
            findings.extend(
                self.check_claim_coinsurance_amount_constraints(
                    carrier_df, "Carrier Claims"
                )
            )
            findings.extend(
                self.check_claim_deductible_amount_constraints(
                    carrier_df, "Carrier Claims"
                )
            )
            findings.extend(
                self.check_claim_primary_payer_paid_amount_constraints(
                    carrier_df, "Carrier Claims"
                )
            )
            findings.extend(
            ...
            findings.extend(
                self.check_claim_outlier_payment_amount_constraints(
                    carrier_df, "Carrier Claims"
                )
            )
            findings.extend(
            ...
            findings.extend(
                self.check_claim_line_coinsurance_amount_constraints(
                    carrier_df, "Carrier Claims"
                )
            )
            findings.extend(
            ...
            findings.extend(
                self.check_claim_line_primary_payer_paid_amount_constraints(
                    carrier_df, "Carrier Claims"
                )
            )
            findings.extend(
                self.check_claim_line_non_covered_charge_amount_constraints(
                    carrier_df, "Carrier Claims"
                )
            )
            findings.extend(
            ...
            findings.extend(
                self.check_claim_line_fourth_hcpcs_modifier_constraints(
                    carrier_df, "Carrier Claims"
                )
            )
            findings.extend(
            ...
            findings.extend(
                self.check_claim_line_revenue_center_rate_amount_constraints(
                    carrier_df, "Carrier Claims"
                )
            )
            if "line_num" in carrier_df.columns:
        ...
        if inpatient_df is not None and not inpatient_df.is_empty():
            ...
            findings.extend(
                self.check_claim_pps_capital_outlier_constraints(
                    inpatient_df, "Inpatient Claims"
                )
            )
            findings.extend(
            ...
            findings.extend(
                self.check_claim_pps_operating_outlier_amount_constraints(
                    inpatient_df, "Inpatient Claims"
                )
            )
            findings.extend(
            ...
            findings.extend(
                self.check_claim_discharge_status_constraints(
                    inpatient_df, "Inpatient Claims"
                )
            )
            findings.extend(
                self.check_claim_admission_type_constraints(
                    inpatient_df, "Inpatient Claims"
                )
            )
            findings.extend(
                self.check_claim_frequency_code_constraints(
                    inpatient_df, "Inpatient Claims"
                )
            )
        ...
        if mbsf_base_df is not None and not mbsf_base_df.is_empty():
            ...
            findings.extend(
                self.check_mbsf_hmo_indicator_constraints(mbsf_base_df)
            )
            findings.extend(
                self.check_record_uniqueness(
