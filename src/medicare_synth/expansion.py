    ...
    def synthesize_table(
        ...
        for idx, col_name in enumerate(cols_to_add):
            if col_name not in result_df.columns:
                ...
                col_expr = (
                    (pl.arange(0, pl.len(), dtype=pl.Int64) * (seed + idx + 1) % 100)
                    / 10.0
                ).alias(col_name)
...
class HorizontalExpander:
    ...
    def expand_subgraph(
        ...
        scaled_tables: Dict[str, List[pl.DataFrame]] = {
            tbl: [df.clone()] for tbl, df in tables.items()
        }
        ...
        return {tbl: pl.concat(dfs, rechunk=True) for tbl, dfs in scaled_tables.items()}
