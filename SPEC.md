...
- **CLI Subcommand Extensions**: Added `catalog`, `diff`, `profile`, `export-ci`, `audit`, and `auto-workflow` subcommands to `src/medicare_synth/cli.py`.
- **Autonomous Workflow Automation Subcommand**: Added `auto-workflow` subcommand in CLI and `run_autonomous_workflow` engine in `src/medicare_synth/workflow.py` to automate verification checks (linter, type checker, tests), git staging/commits, pushing, PR creation via GitHub CLI, and autonomous merging.
- **Adoption & Maintenance Test Suite**: Added unit test suite in `tests/test_mbsf_ffs.py`, `tests/test_autonomous_workflow.py`, and `tests/test_cli.py` (149 total passing unit tests).
